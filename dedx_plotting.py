...
plot_title = "Generated Sample1 dE/dx"
...
run1_dEdx = np.load(analysis_dir+run1_name+"_dEdxs.npy")
run1_legend += " ("+str(len(run1_lengths))+")"

if plot_avg_line:
	run2_lengths = np.load(analysis_dir+run2_run+"_lengths.npy") 
	run2_dEdx = np.load(analysis_dir+run2_run+"_dEdxs.npy")
...
if not plot_avg_line:
	exit() 

### Add The Average Line from Run 2 ### 
